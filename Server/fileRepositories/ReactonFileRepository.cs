...
public class ReactionFileRepository : IReactionRepository
{
    ...
    public async Task<Reaction> AddAsync(Reaction reaction)
    {
        ...
        if (reactions.Any(r => r.OnPost == reaction.OnPost && r.ByUser == reaction.ByUser && r.Type == reaction.Type))
        {
    ...
    public async Task DeleteAsync(Reaction reaction)
    {
        ...
        Reaction? reactionToRemove = reactions.SingleOrDefault(r =>
            r.OnPost == reaction.OnPost && r.ByUser == reaction.ByUser && r.Type == reaction.Type);
    ...
    public async Task DeleteAllAsync(Post post)
    {
        ...
        reactions.RemoveAll(r => r.OnPost.PostId == post.PostId);
    ...
    public async Task DeleteAllAsync(User user)
    {
        ...
        reactions.RemoveAll(r => r.ByUser.UserId == user.UserId);
    ...
    {
        List<Reaction> reactions = await LoadListFromFileAsync();
        return reactions.Count(r => r.OnPost.PostId == post.PostId && r.Type == type);
    }
